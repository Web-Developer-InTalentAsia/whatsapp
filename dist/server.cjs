...
var JWT_SECRET = process.env.JWT_SECRET || "intalent_whatsapp_secret_key";
...
async function ensureSeedData() {
  try {
    ...
    if (existingUsers.length === 0) {
      ...
      const hashedSuperAdmin = import_bcryptjs.default.hashSync("adminpassword", 10);
      const [superAdmin] = await db.insert(schema_exports.users).values({
      ...
      const hashedAdmin = import_bcryptjs.default.hashSync("admin123", 10);
      const [adminUser] = await db.insert(schema_exports.users).values({
      ...
      const hashedAgent = import_bcryptjs.default.hashSync("user123", 10);
      const [agentUser] = await db.insert(schema_exports.users).values({
      ...
      const [num1] = await db.insert(schema_exports.whatsappNumbers).values({
      ...
      const [num2] = await db.insert(schema_exports.whatsappNumbers).values({
      ...
      await db.insert(schema_exports.workflows).values({
        whatsappNumberId: num1.id,
      ...
      const [contact1] = await db.insert(schema_exports.contacts).values({
      ...
      const [contact2] = await db.insert(schema_exports.contacts).values({
      ...
      const [conv1] = await db.insert(schema_exports.conversations).values({
        contactId: contact1.id,
        whatsappNumberId: num1.id,
        assignedUserId: agentUser.id,
        status: "open",
        lastMessageAt: new Date(Date.now() - 30 * 6e4)
      }).returning();
      const [conv2] = await db.insert(schema_exports.conversations).values({
        contactId: contact2.id,
        whatsappNumberId: num1.id,
        assignedUserId: adminUser.id,
        status: "unread",
        lastMessageAt: new Date(Date.now() - 5 * 6e4)
      }).returning();
      await db.insert(schema_exports.messages).values([
        ...
        },
        {
          ...
          replyType: "manual",
          status: "sent",
          timestamp: new Date(Date.now() - 100 * 6e4),
...
var authenticateJWT = async (req, res, next) => {
  ...
  try {
    ...
    const [user] = await db.select().from(schema_exports.users).where((0, import_drizzle_orm2.eq)(schema_exports.users.id, decoded.id)).limit(1);
...
app.post("/api/whatsapp_numbers/:id/test-connection", authenticateJWT, async (req, res) => {
  ...
  try {
    ...
    const [updated] = await db.update(schema_exports.whatsappNumbers).set({ webhookStatus: "Verified", lastVerified: /* @__PURE__ */ new Date() }).where((0, import_drizzle_orm2.eq)(schema_exports.whatsappNumbers.id, parseInt(id))).returning();
